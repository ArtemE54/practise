...
class BinaryTree:
    ...
    def _in_order_recursively(self, node):
        return (self._in_order_recursively(node.left) if node else []) + \
               [node.data] + \
               (self._in_order_recursively(node.right) if node else [])
    ...
    def _pre_order_recursively(self, node):
        return [node.data] + \
               (self._pre_order_recursively(node.left) if node else []) + \
               (self._pre_order_recursively(node.right) if node else [])

    def post_order_traversal(self):
        return self._post_order_recursively(self.root)

    def _post_order_recursively(self, node):
        return (self._post_order_recursively(node.left) if node else []) + \
               (self._post_order_recursively(node.right) if node else []) + \
               [node.data]
